...
const OfflineDB = (function () {
    ...
    // v2 added the "actions" store (deferred internet-only commands, e.g.
    ...
    function openSchoolDb(schoolId) {
        return openDb(`srs_offline_school_${schoolId}`, SCHOOL_DB_VERSION, (db) => {
            ...
            if (!db.objectStoreNames.contains("actions")) {
                const store = db.createObjectStore("actions", { keyPath: "client_uuid" });
                store.createIndex("sync_status", "_sync.status");
            }
        });
    }
    ...
    async function deleteRecord(schoolId, entity, clientUuid) {
        const db = await openSchoolDb(schoolId);
        await tx(db, entity, "readwrite", (t) => t.objectStore(entity).delete(clientUuid));
        db.close();
    }
    ...
    async function getAll(schoolId, entity, opts) {
        const db = await openSchoolDb(schoolId);
        const all = await reqToPromise(db.transaction(entity, "readonly").objectStore(entity).getAll());
        db.close();
        return opts && opts.includeDeleted ? all : all.filter((r) => !r.is_deleted);
    }
    ...
    return {
        ...
        putRecord, putRecords, deleteRecord, getRecord, getAll, getByStatus,

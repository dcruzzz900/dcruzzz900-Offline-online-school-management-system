...
def generate_tenant_id(conn):
    """Generate a permanent, non-secret tenant identifier."""
    alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"
    for _ in range(100):
        value = "TEN-" + "".join(_secrets.choice(alphabet) for _ in range(8))
        if not conn.execute("SELECT 1 FROM schools WHERE tenant_id=?", (value,)).fetchone():
            return value
    raise RuntimeError("Could not generate a unique tenant ID")


def generate_school_id(conn, school_name):
    """Generate a stable human-readable school ID; never reused."""
    base = re.sub(r"[^A-Z0-9]+", "", (school_name or "").upper())[:18] or "SCHOOL"
    candidate = base
    n = 2
    while conn.execute("SELECT 1 FROM schools WHERE school_code=?", (candidate,)).fetchone():
        suffix = str(n)
        candidate = (base[:max(1, 20-len(suffix))] + suffix)
        n += 1
    return candidate


def ensure_school_identity(conn, school_id, school_name=None):
    row = conn.execute("SELECT tenant_id, school_code, name FROM schools WHERE id=?", (school_id,)).fetchone()
    if not row:
        return None
    tenant_id = row["tenant_id"] or generate_tenant_id(conn)
    school_code = row["school_code"] or generate_school_id(conn, school_name or row["name"])
    conn.execute("UPDATE schools SET tenant_id=?, school_code=? WHERE id=?",
                 (tenant_id, school_code, school_id))
    return tenant_id, school_code


def tenant_for_user(conn, user_id):
    row = conn.execute(
        "SELECT u.id, u.school_id, u.tenant_id, s.tenant_id AS school_tenant_id, "
        "u.role, u.position, u.is_active, s.activation_status, s.is_suspended, s.is_archived "
        "FROM users u JOIN schools s ON s.id=u.school_id WHERE u.id=?", (user_id,)
    ).fetchone()
    if not row or not row["is_active"] or row["activation_status"] != "active" or row["is_suspended"] or row["is_archived"]:
        return None
    return row
...
def migration_039_multi_tenant_rbac(conn):
    """Permanent tenant identity + school code + scoped role assignments.
    Existing school_id isolation remains for backward compatibility; tenant_id
    is the canonical tenant boundary and is server-derived."""
    if not table_exists(conn, "schools"):
        return

    ensure_column(conn, "schools", "tenant_id", "TEXT")
    ensure_column(conn, "schools", "school_code", "TEXT")
    ensure_column(conn, "schools", "school_level", "TEXT NOT NULL DEFAULT 'secondary'")
    conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_schools_tenant_id ON schools(tenant_id)")
    conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_schools_school_code ON schools(school_code)")

    # Permanent identities for existing schools.
    rows = conn.execute("SELECT id, name, tenant_id, school_code FROM schools ORDER BY id").fetchall()
    for r in rows:
        if not r["tenant_id"] or not r["school_code"]:
            tenant = r["tenant_id"] or generate_tenant_id(conn)
            code = r["school_code"] or generate_school_id(conn, r["name"])
            conn.execute("UPDATE schools SET tenant_id=?, school_code=? WHERE id=?", (tenant, code, r["id"]))

    # Add tenant_id to all school-owned tables that exist in this build.
    tables = [
        "notifications","audit_log","users","sessions","terms","classes","subjects",
        "class_subjects","students","enrollments","grading_config","grade_scale",
        "scores","score_history","student_term_info","skill_traits","student_skill_ratings",
        "attendance_records","materials","staff_attendance","activation_codes",
        "device_credentials","sync_conflicts","sync_log","deferred_actions",
        "change_audit","sync_tombstones","offline_sync_tokens",
        "timetable_periods","timetable_entries"
    ]
    for t in tables:
        if table_exists(conn, t):
            ensure_column(conn, t, "tenant_id", "TEXT")

    # Direct school_id backfill.
    for t in tables:
        if table_exists(conn, t) and "school_id" in column_names(conn, t):
            conn.execute(f"""
                UPDATE {t}
                   SET tenant_id=(SELECT tenant_id FROM schools s WHERE s.id={t}.school_id)
                 WHERE tenant_id IS NULL AND {t}.school_id IS NOT NULL
            """)

    # Relationship-derived tenant backfill.
    rels = [
        ("terms","sessions","session_id"), ("class_subjects","classes","class_id"),
        ("students","classes","class_id"), ("enrollments","classes","class_id"),
        ("scores","students","student_id"), ("scores","students","student_id"),
        ("score_history","scores","score_id"), ("student_term_info","students","student_id"),
        ("student_skill_ratings","students","student_id"),
        ("attendance_records","classes","class_id"),
        ("timetable_entries","classes","class_id"),
    ]
    for child,parent,fk in rels:
        if table_exists(conn, child) and table_exists(conn, parent) and fk in column_names(conn,child):
            conn.execute(f"""
                UPDATE {child}
                   SET tenant_id=(SELECT tenant_id FROM {parent} p WHERE p.id={child}.{fk})
                 WHERE {child}.tenant_id IS NULL
            """)

    # Server-enforced tenant stamping. Client-supplied tenant_id is overwritten
    # from the authoritative school relationship.
    for t in tables:
        if not table_exists(conn,t): continue
        cols=column_names(conn,t)
        if "tenant_id" not in cols: continue
        school_expr = f"(SELECT tenant_id FROM schools WHERE id=NEW.school_id)" if "school_id" in cols else None
        if school_expr:
            conn.execute(f"""
                CREATE TRIGGER IF NOT EXISTS trg_{t}_tenant_insert
                AFTER INSERT ON {t} FOR EACH ROW
                WHEN NEW.school_id IS NOT NULL
                BEGIN UPDATE {t} SET tenant_id={school_expr} WHERE rowid=NEW.rowid; END
            """)
            conn.execute(f"""
                CREATE TRIGGER IF NOT EXISTS trg_{t}_tenant_update
                AFTER UPDATE OF school_id, tenant_id ON {t} FOR EACH ROW
                WHEN NEW.school_id IS NOT NULL
                BEGIN UPDATE {t} SET tenant_id={school_expr} WHERE rowid=NEW.rowid; END
            """)

    # Relationship triggers for tables without direct school_id.
    relationship_triggers = [
        ("terms","sessions","session_id"),("class_subjects","classes","class_id"),
        ("students","classes","class_id"),("enrollments","classes","class_id"),
        ("scores","students","student_id"),("student_term_info","students","student_id"),
        ("student_skill_ratings","students","student_id"),("attendance_records","classes","class_id"),
        ("timetable_entries","classes","class_id"),("score_history","scores","score_id"),
    ]
    for child,parent,fk in relationship_triggers:
        if not (table_exists(conn,child) and table_exists(conn,parent)): continue
        if "tenant_id" not in column_names(conn,child) or fk not in column_names(conn,child): continue
        conn.execute(f"""
            CREATE TRIGGER IF NOT EXISTS trg_{child}_tenant_rel_insert
            AFTER INSERT ON {child} FOR EACH ROW
            WHEN NEW.{fk} IS NOT NULL
            BEGIN UPDATE {child} SET tenant_id=(SELECT tenant_id FROM {parent} WHERE id=NEW.{fk}) WHERE rowid=NEW.rowid; END
        """)
        conn.execute(f"""
            CREATE TRIGGER IF NOT EXISTS trg_{child}_tenant_rel_update
            AFTER UPDATE OF {fk}, tenant_id ON {child} FOR EACH ROW
            WHEN NEW.{fk} IS NOT NULL
            BEGIN UPDATE {child} SET tenant_id=(SELECT tenant_id FROM {parent} WHERE id=NEW.{fk}) WHERE rowid=NEW.rowid; END
        """)

    # Role-assignment model. Legacy role/position fields remain for compatibility.
    conn.execute("""
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_role_assignments_tenant_user ON role_assignments(tenant_id,user_id,status)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_role_assignments_scope ON role_assignments(tenant_id,school_level,department,class_id,subject_id)")
    conn.execute("""
        CREATE TABLE IF NOT EXISTS role_assignment_audit (
            ...
            new_permissions_json TEXT,
            previous_scope_json TEXT,
            new_scope_json TEXT,
            changed_by INTEGER,
    ...
    # Ensure role tables are tenant stamped if created during this migration.
    conn.commit()
...
STEPS = [
    ...
    ("multi_tenant_rbac", migration_039_multi_tenant_rbac),
]
